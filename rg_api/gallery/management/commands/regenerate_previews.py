...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--clean', action='store_true', help='Delete existing previews before regenerating')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Found {total_images} images. Starting regeneration...')

        for index, image_obj in enumerate(images):
            # Check if source file exists
            if not image_obj.image or not os.path.exists(image_obj.image.path):
                self.stdout.write(self.style.WARNING(f'Source image not found for ID {image_obj.pk}: {image_obj.title}'))
                continue
            ...
            try:
                ...
                if img is None:
                     self.stdout.write(self.style.WARNING(f'Could not read image ID {image_obj.pk} with OpenCV'))
                     continue

                original_height, original_width = img.shape[:2]

                for width in widths:
                    filename = os.path.join(preview_dir, f"{image_obj.pk}_{width}.webp")
                    ...
                    _, im_buf_arr = cv2.imencode(".webp", resize, [int(cv2.IMWRITE_WEBP_QUALITY), quality])
                ...
                if (index + 1) % 10 == 0:
                     self.stdout.write(f'Processed {index + 1}/{total_images} images...')

            except Exception as e:
                self.stdout.write(self.style.ERROR(f'Error processing image ID {image_obj.pk}: {e}'))

        self.stdout.write(self.style.SUCCESS('Successfully regenerated all previews.'))
