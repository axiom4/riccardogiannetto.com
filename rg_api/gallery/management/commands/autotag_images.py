...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING(f"Found {total} images to process (Force={force})"))
        ...
        for image_obj in queryset:
            ...
            if not image_obj.image:
                self.stdout.write(self.style.WARNING(f"Skipping ID {image_obj.id}: No image file"))
                continue
            ...
            try:
                ...
                self.stdout.write(f"Processing ID {image_obj.id}: {image_obj.title}...")
                
                tags = classify_image(path)
                
                if tags:
                    image_obj.tags.add(*tags)
                    self.stdout.write(self.style.SUCCESS(f"  + Added tags: {', '.join(tags)}"))
                else:
                    self.stdout.write(self.style.WARNING("  - No tags generated"))
                
                processed_count += 1

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error processing ID {image_obj.id}: {e}"))

        self.stdout.write(self.style.SUCCESS(f"Done. Processed {processed_count} images."))
