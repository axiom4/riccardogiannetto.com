...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Add Root
        urlset.append(f'  <url><loc>{base_url}/</loc><lastmod>{current_date_iso}</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>')
        
        # Add Static Pages like Map
        urlset.append(f'  <url><loc>{base_url}/map</loc><lastmod>{current_date_iso}</lastmod><changefreq>weekly</changefreq><priority>0.8</priority></url>')
        ...
        for image in images:
            ...
            lastmod = image.updated_at.strftime('%Y-%m-%d') if image.updated_at else current_date_iso
        ...
        count_posts = 0
        for post in posts:
            lastmod = post.updated_at.strftime('%Y-%m-%d') if post.updated_at else current_date_iso
        ...
        count_pages = 0
        for page in pages:
            lastmod = page.updated_at.strftime('%Y-%m-%d') if page.updated_at else current_date_iso
        ...
        try:
            if not output_dir.exists():
                 self.stdout.write(self.style.WARNING(f'Directory {output_dir} does not exist. Creating it if possible or failing.'))
                 # Typically we don't create src folder, it should exist.
            
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write('\n'.join(urlset))
            
            self.stdout.write(self.style.SUCCESS(f'Successfully generated sitemap at {output_file}'))
            self.stdout.write(f'Included: {count_images} images, {count_posts} posts, {count_pages} pages.')
