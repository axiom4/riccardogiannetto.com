...
if (!fs.existsSync(INDEX_PATH)) {
    ...
    if (!fs.existsSync(INDEX_PATH)) {
        console.error(`Error: Cannot find index.html in ${BASE_DIST} or ${path.join(BASE_DIST, 'browser')}. Did you run 'ng build'?`);
        process.exit(1);
    }
}
...
$('script').each((i, elem) => {
  ...
  if (src && !src.startsWith('http') && !src.startsWith('//')) { // Only local files
...
$('link[rel="stylesheet"], link[rel="modulepreload"]').each((i, elem) => {
  ...
  if (href && !href.startsWith('http') && !href.startsWith('//')) { // Only local files
